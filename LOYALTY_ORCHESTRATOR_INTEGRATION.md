...
9. [Example C — Earn + Burn Simultaneously (Split)](#9-example-c--earn--burn-simultaneously-split)
...
(`MAM`) — exposing each program's authentication and redemption behaviour through one uniform
interface. It supports **earn** (accrue), **burn** (redeem), **split** (earn + burn on one order),
full or partial reversal, status query, reconciliation, and signed-JWT webhooks.
...
| **Partner auth** | `X-API-Key` + `X-Partner-Code` (+ optional mTLS in production) |
...
[Split](#9-example-c--earn--burn-simultaneously-split).
...
|-----------|------------------------|----------------------------|
| **Burn (Redeem)** | **Always** (once) | Only if the program demands: SAS = OTP every burn; M&M = redirect every burn; FLB/Etihad = none (Etihad OTP only if configured). |
...
[Example C](#9-example-c--earn--burn-simultaneously-split).

---

## 4. Per-Program Capability

Capability is expressed with **two orthogonal axes plus one derived flag**, because a single label
can't capture the central nuance: *a program can need a redirect to authenticate but no redirect to
burn.* Query it at `GET /loyalty/v1/programs`.

- **`authMode`** — how member context is established, **once**: `MEMBER_OAUTH_REDIRECT`,
  `MEMBER_SAML_SSO`, or `B2B_MACHINE_TOKEN`.
- **`burnStep`** — what each individual burn needs beyond the call: `NONE`, `OTP_CHALLENGE`, or
  `EXTERNAL_REDIRECT`.
...
No program is zero-redirect for a brand-new member — each needs a one-time login. *After* that:
Flying Blue and Etihad burn server-side with no redirect (Etihad subject to its OTP config); SAS
challenges an OTP every burn; Miles & More needs a per-transaction MCheckout redirect. This is never
hard-coded — `quote` returns `oneClickEligible` and the concrete next step per member.
...
- The key is **redacted** from Pointspay logs and error output. Store it as a server secret; rotate
  via the onboarding contact.

### 5.2 Member Context

To act on a member's points, first establish a **member context** — **once** — via a single endpoint
that hides each program's mechanics (OAuth for FLB/MAM, SAML SSO for Etihad, machine token for SAS
B2B). No SAML or OAuth implementation is required on the client side.
...
2. **Poll (works everywhere; also the deep-link fallback).** Poll
   `GET /auth/member-context/{contextRef}` until `status` is `READY` (or `EXPIRED`). Suggested
   interval ≈ 2 s, timeout at `expiresAt`. Most robust for embedded web-views.
3. **`member-context.ready` webhook (push).** A signed JWT sent to your backend carrying the
   `memberToken` + `memberRef` directly (§11) — no `POST .../token` call needed.

> **Security — the token never travels through the browser.** The redirect carries only `contextRef`
> + `status`. The `memberToken` reaches your **backend** only, either as a claim in the signed
...
> Either way it stays out of the web-view, URL history, deep-link payloads, and logs — the same
> reason OAuth returns a code, not a token, in the redirect.
...
`READY` / `EXPIRED`, used for polling above); `POST /auth/member-context/{contextRef}/token`
(obtain the member token once `READY`, and refresh it later — the pull alternative to the webhook);
`DELETE …/token` (revoke); `GET …/introspect` (inspect validity/scope/expiry). **One member context
is reused for many operations** — including both legs of a split.
...
(sandbox). Every call carries `X-API-Key` + `X-Partner-Code`.
...
| 7 | `/redemptions/{operationReference}/otp` | POST | ✅ | ✅ | Submit OTP (SAS; Etihad if configured) | — | (2) |
...
Earn (accrue) points for a member — a single server-to-server call. The member authenticated once
...
> points are never double-credited.

---

## 8. Example B — Burn

Burn points to pay for (part of) an order (Scenario 3 = full burn). The member logs in **once**; the
burn then follows the program's `burnStep`.
...
(2) echo the `quoteRef` on the burn so pricing/eligibility **can't drift** between quote and burn
...
## 9. Example C — Earn + Burn Simultaneously (Split)
...
**Orchestration & failure handling** (the client sequences the three legs):

| Failure point | Recommended action |
|---------------|--------------------|
| **Burn fails** | Do not charge the card. Surface the error to the user. |
| **Card charge fails** (after burn) | Call `POST /redemptions/{op_burn}/reverse` to return the points; cancel the order. |
...
> Both legs ran under the **one** member token from step 1. The member authenticated exactly once; the
> only time an extra step appears is if the program demands a per-burn OTP/redirect (§3).

---

## 10. Status Lifecycle

Every earn and burn carries a `status`. Statuses are **monotonic** — they never move backwards, and
terminal states are immutable. Read the current state from `GET /redemptions/{operationReference}` or
`GET /accruals/{operationReference}`, or receive it via webhook (§11).
...
| `PARTIALLY_REVERSED` | Some — but not all — burned points returned. | — (more reversals possible) |
...
| `PARTIALLY_REVERSED` | Some — but not all — earned points clawed back. | — |
...
| `statusUpdates[]` | `timestamp`, `message`, `source` (`LOYALTY`/`INTERNAL`) — progress/error notes. | `status_updates[]` |

`status` is derived from `reversedPoints`: `0` → `REDEEMED`/`ACCRUED`; `0 < reversedPoints < points`
→ `PARTIALLY_REVERSED`; `== points` → `REVERSED`. Poll the operation (or a specific
...
- **`member-context.ready`** — after a redirect auth completes; carries the `memberToken` +
  `memberRef` to your backend (the push alternative to `POST .../token`; §5.2).
- **operation settled-state** — when an earn/burn settles (`REDEEMED`, `REDEMPTION_FAILED`,
...
| Algorithms | `RS256` / `RS384` / `RS512` (whitelist; reject `alg: none`). |
| Audience (`aud`) | your partner code. |

**Claim fields** — compact keys, following the same convention as Pointspay payment tokens (e.g. V5's `stat` / `oid`):

| Claim | Meaning |
|-------|---------|
| `iss`, `aud`, `iat`, `exp` | Standard JWT claims — issuer, audience, issued-at, expiry. |
| `jti` | Unique event id — dedup on it (anti-replay; also reject if `exp` past or `iat` older than 5 min). |
...
Reversals are initiated via the partner key — **no member interaction** — and may be **full or
partial**.

**Full reversal** (omit `points` — reverses the full remaining amount):
...
**Partial reversal** — mirror a partial refund by reversing only the affected share. Partials
accumulate up to the original; the operation sits at `PARTIALLY_REVERSED` until fully reversed:
...
| **Partial** | Supply `points` ≤ remaining (`points − reversedPoints`). Multiple partials accumulate; `status` stays `PARTIALLY_REVERSED` until fully reversed (see §10). |
| **Both directions** | `…/redemptions/{op}/reverse` returns burned points; `…/accruals/{op}/reverse` claws back earned points. |
| **Idempotent** | Each call is idempotent on `X-Idempotency-Key`; a retry never double-reverses. |
...
**Idempotency** — required on every mutating call (`/accruals`, `/redemptions`, `…/otp`,
`…/reverse`) via the **`X-Idempotency-Key`** header (≥16 chars):
...
| 5 | **Webhook URL** | HTTPS endpoint for `member-context.ready` + settled-state signed-JWT events | ☐ |
...
| 8 | **Sandbox access** | UAT base URL + test member accounts with known balances | ☐ |
